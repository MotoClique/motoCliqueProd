...
module.exports.updateBuy = function(req,res){//Update
	...
	Buy.findOneAndUpdate({_id:doc._id},{$set: doc},{},(err, updated)=>{
		if(err){
			...
			if(doc.active === '-'){
				ChatInbox.update({post_id: doc.buy_req_id}, {"$set": {post_deletion: true}}, {multi: true}, (updateChat_err, updateChat_res)=>{ });
				Fav.remove({bid_sell_buy_id: doc.buy_req_id}, function(fav_err,fav_result){ });
			}
